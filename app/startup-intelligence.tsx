...
type Props = { startupId: string; gmailToken: string; onClose: () => void; onTokenExpired: () => void };
...
export default function StartupIntelligencePanel({ startupId, gmailToken, onClose, onTokenExpired }: Props) {
  ...
  async function discoverFounderEmails(founderName?: string) {
    ...
    try {
      ...
      setNotice(usable ? `Verified founder contact selected for ${usable.founderName}. Review the draft before sending.` : result.providerConfigured ? "No safely verified founder email was returned. Guessed candidates remain disabled." : "Email patterns were generated, but HUNTER_API_KEY is not configured, so no guessed address can be used.");
    } catch (error) { handleError(error); }
    finally { setBusy(""); }
  }
