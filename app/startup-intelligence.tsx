  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        setNotice(usable ? `Verified founder contact selected for ${usable.founderName}.` : result.providerConfigured ? "Research is ready, but no safely verified founder email was returned." : "Research is ready. Email patterns were generated, but HUNTER_API_KEY is required to verify them before use.");
      } catch (error) { handleError(error); }
      finally { setBusy(""); }
    }, 0);
    return () => window.clearTimeout(timer);
