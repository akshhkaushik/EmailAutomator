...
export default function StartupIntelligencePanel({ startupId, gmailToken, onClose, onTokenExpired }: Props) {
  ...
  const load = useCallback(async () => {
    setBusy("loading");
    try {
      const [intelligenceResult, contributionResult, buildResult, outreachResult] = await Promise.all([
  ...
  async function draftOutreach(opportunityId: string) {
    ...
    try { const result = await request<{ draft: OutreachDraftAudit }>(`/api/startups/${startupId}/outreach`, gmailToken, "POST", { opportunityId, mode: outreachMode, recipientEmail: recipient.email, recipientName: recipient.name }); setActiveDraft(result.draft); setOutreachDrafts((current) => [result.draft, ...current]); }
    catch (error) { handleError(error); } finally { setBusy(""); }
  }
