...
export default function StartupIntelligencePanel({ startupId, gmailToken, autoResearch = false, onClose, onTokenExpired }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        setNotice(usable ? `Verified founder contact selected for ${usable.founderName}.` : "Research is ready, but the native evidence engine did not find enough mailbox evidence to unlock sending.");
      } catch (error) { handleError(error); }
      finally { setBusy(""); }
    }, 0);
    return () => window.clearTimeout(timer);
  ...
  async function discoverFounderEmails(founderName?: string) {
    ...
    try {
      ...
      const usable = result.contacts.find((contact) => contact.email && (contact.verificationStatus === "valid" || (contact.verificationStatus === "accept_all" && contact.confidence >= 85)));
      if (usable?.email) setRecipient({ email: usable.email, name: usable.founderName, contactId: usable.id });
      setNotice(usable ? `Verified founder contact selected for ${usable.founderName}. Review the draft before sending.` : "No safely verified founder email was returned. Ranked candidates and their evidence remain inspectable, but sending stays disabled.");
    } catch (error) { handleError(error); }
    finally { setBusy(""); }
  }
  ...
  return <section className="panel intelligence-panel" aria-label="Startup intelligence detail">
    ...
    : <>
      ...
      <section className="workflow-section contact-discovery"><div className="contribution-heading"><div><p className="kicker">Founder contact</p><h2>Find a verified company email.</h2><p>Searches bounded public company pages, learns observed address formats, checks mail-domain health, and optionally asks external verifiers for corroboration.</p></div><button className="secondary-button" type="button" disabled={Boolean(busy) || intelligence.founders.value === "unknown"} onClick={() => void discoverFounderEmails()}>{busy === "contacts" ? "Checking…" : contacts.length ? "Check again" : "Find founder emails"}</button></div><div className="accuracy-note"><b>Native evidence engine active:</b> an exact publicly published professional address can be verified without Hunter. Patterns and DNS improve ranking but never unlock sending alone.</div><div className="contact-list">{contacts.map((contact) => { const usable = Boolean(contact.email) && (contact.verificationStatus === "valid" || (contact.verificationStatus === "accept_all" && contact.confidence >= 85)); return <article className="contact-card" key={contact.id}><header><div><h3>{contact.founderName}</h3><p>{contact.founderRole}</p></div><span className={`contact-status status-${contact.verificationStatus}`}>{contact.verificationStatus.replace("_", "-")}</span></header>{contact.email ? <p className="contact-email">{contact.email}</p> : <p className="unknown-copy">No verified address found.</p>}<p>{contact.origin === "public" ? `Publicly sourced from ${contact.sourceUrls.length} page${contact.sourceUrls.length === 1 ? "" : "s"}.` : contact.origin === "inferred" ? `Pattern-inferred and checked by ${contact.provider}.` : `${contact.candidates.length} likely combinations generated but not treated as facts.`}</p><details><summary>Inspect ordered evidence</summary><ol>{contact.candidates.map((candidate) => <li key={candidate.email}><b>#{candidate.rank} {candidate.pattern}</b>: {candidate.email} · {candidate.verificationStatus}{candidate.confidence ? ` (${candidate.confidence}%)` : ""}{(candidate.evidence || []).length > 0 && <small> — {(candidate.evidence || []).map((item) => item.detail).join(" ")}</small>}</li>)}</ol>{contact.sourceUrls.map((source) => <a key={source.url} href={source.url} target="_blank" rel="noreferrer">Public source ↗</a>)}</details><button type="button" disabled={!usable} onClick={() => selectContact(contact)}>{recipient.contactId === contact.id ? "Selected" : usable ? "Use for outreach" : "Verification required"}</button></article>; })}</div></section>
